...
impl DiscordChannel {
    ...
    async fn send_approval_feedback(
        ...
        let result = if approved {
            approval_formatter::ApprovalResult::Approved
        } else {
            approval_formatter::ApprovalResult::Rejected
        };
        let feedback = approval_formatter::format_approval_result(result, "", None);
                                            ...
                                            discord_event_to_inbound_with_attachments(d)
                                        {
                                            ...
                                            let (trigger_agent, skip_send) = if !enable_allowlist {
                                                ...
                                                let pairings = config_path
                                                    .as_ref()
                                                    .map(|p| {
                                                        pairings_from_config_file_cached(p.as_path())
                                                    })
                                                    .unwrap_or_default();
                                                let paired = pairing_allows(
                                                    &inbound.chat_id,
                                                    channel_provider,
                                                    &pairings,
                                                );
                                                ...
                                                match entry {
                                                    ...
                                                    None => {
                                                        if is_group {
                                                            if let Some(my_name) = group_my_name {
                                                                let trimmed =
                                                                    inbound.content.trim_start();
                                                                let mention_prefix =
                                                                    format!("<@!{}>", my_name);
                                                                let mention_prefix_alt =
                                                                    format!("<@{}>", my_name);
                                                                let starts = trimmed
                                                                    .starts_with(&mention_prefix)
                                                                    || trimmed.starts_with(
                                                                        &mention_prefix_alt,
                                                                    );
                                                                if !starts {
                                            ...
                                            if let Some((request_id, _)) = pending.remove(&inbound.sender_id) {
                                                drop(pending);
                                                ...
                                                if let Some(obj) = meta.as_object_mut() {
                                                    obj.insert("pending_approval_request_id".into(), serde_json::Value::String(request_id));
                                                }
                                                let inbound_with_meta = crate::bus::InboundMessage {
                                                    metadata: meta,
                                                    ..inbound
                                                };
