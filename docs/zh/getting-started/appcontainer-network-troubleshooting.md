...
为 AppContainer 添加防火墙出站规则和 WFP 放行需要**管理员**权限。规则是**持久化**的：沙箱停止时**不会**被删除。为避免以管理员身份运行完整 Synbot 守护进程，建议：

- **安装后或重启后首次运行**：以**管理员身份**运行**一次**（例如右键 → 以管理员身份运行）：
  ```bash
  synbot sandbox setup
  ```
  该命令仅添加防火墙与 WFP 规则后即退出，**不会**启动守护进程。无需以管理员身份运行 `synbot sandbox start`。
- **之后的运行**：任意用户（包括普通用户）均可运行 `synbot sandbox start`；已有规则会放行 AppContainer 的出站网络。
- **重启之后**：Synbot 添加的 WFP 筛选器为**持久筛选器**（带 `FWPM_FILTER_FLAG_PERSISTENT`），会由 BFE 在重启后自动恢复，通常无需再次运行 `setup`。若防火墙规则被系统或策略清除，或出站仍不通，再以管理员运行一次 `synbot sandbox setup` 即可。若规则曾被手动删除，同样以管理员运行一次 `setup` 重新添加。
