...
新安装默认使用 `safe`。安全模式会开启 Exec 审批、限制工作区路径，并优先创建工具沙盒；如果沙盒不可用，Exec 会拒绝执行而不会回退到宿主机。

可选预设：

| 模式 | 行为 |
|---|---|
| `safe` | 开启权限审批和工作区限制，优先使用工具沙盒；显式配置的字段仍以用户配置为准 |
| `developer` | 开启审批和工作区限制，自动允许常见低风险命令，其余命令需要审批 |
| `trusted` | 用户明确选择后的兼容模式，不注入新的安全限制，适合本地实验 |
...
已有配置如果没有 `securityProfile`，配置迁移会标记为 `trusted`，保留原有行为，不会静默升级权限限制。配置版本会升级到 2，并在原文件旁保留迁移备份。
